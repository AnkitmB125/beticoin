...
def make_utxo(node, amount, confirmed=True, scriptPubKey=DUMMY_P2WPKH_SCRIPT):
    ...
    """
    fee = 1*COIN
    while node.getbalance() < betishi_round((amount + fee)/COIN):
        node.generate(100)

    new_addr = node.getnewaddress()
    txid = node.sendtoaddress(new_addr, betishi_round((amount+fee)/COIN))
...
class ReplaceByFeeTest(beticoinTestFramework):
