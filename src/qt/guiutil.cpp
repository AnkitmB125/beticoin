...
#include <qt/beticoinaddressvalidator.h>
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a beticoin address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(Params()))));
    widget->setValidator(new beticoinAddressEntryValidator(parent));
    widget->setCheckValidator(new beticoinAddressCheckValidator(parent));
}

bool parsebeticoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no beticoin: URI
    if(!uri.isValid() || uri.scheme() != QString("beticoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!beticoinUnits::parse(beticoinUnits::BTC, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsebeticoinURI(QString uri, SendCoinsRecipient *out)
{
    QUrl uriInstance(uri);
    return parsebeticoinURI(uriInstance, out);
}

QString formatbeticoinURI(const SendCoinsRecipient &info)
{
    bool bech_32 = info.address.startsWith(QString::fromStdString(Params().Bech32HRP() + "1"));

    QString ret = QString("beticoin:%1").arg(bech_32 ? info.address.toUpper() : info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(beticoinUnits::format(beticoinUnits::BTC, info.amount, false, beticoinUnits::SeparatorStyle::NEVER));
        paramCount++;
    }
...
bool openbeticoinConf()
{
    fs::path pathConfig = GetConfigFile(gArgs.GetArg("-conf", beticoin_CONF_FILENAME));
    ...
    /* Open beticoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "beticoin.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "beticoin (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("beticoin (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for beticoin*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = gArgs.GetChainName();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "beticoin.desktop";
    return GetAutostartDir() / strprintf("beticoin-%s.desktop", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = gArgs.GetChainName();
        // Write a beticoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=beticoin\n";
        else
            optionFile << strprintf("Name=beticoin (%s)\n", chain);
