...
const char * const beticoin_CONF_FILENAME = "beticoin.conf";
const char * const beticoin_SETTINGS_FILENAME = "settings.json";
 ...
 * https://github.com/beticoin/beticoin/pull/16097#issuecomment-514627823
 */
...
bool ArgsManager::ParseParameters(int argc, const char* const argv[], std::string& error)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if (key == "-") break; //beticoin-tx using stdin
        std::string val;
...
bool ArgsManager::GetSettingsPath(fs::path* filepath, bool temp) const
{
    ...
    if (filepath) {
        std::string settings = GetArg("-settings", beticoin_SETTINGS_FILENAME);
...
{
    // Windows: C:\Users\Username\AppData\Roaming\beticoin
    // macOS: ~/Library/Application Support/beticoin
    // Unix-like: ~/.beticoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "beticoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // macOS
    return pathRet / "Library/Application Support/beticoin";
#else
    // Unix-like
    return pathRet / ".beticoin";
#endif
#endif
}
...
bool ArgsManager::ReadConfigFiles(std::string& error, bool ignore_invalid_keys)
{
    ...
    const std::string confPath = GetArg("-conf", beticoin_CONF_FILENAME);
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Make sure beticoin Core copyright is not removed by accident
    if (copyright_devs.find("beticoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The beticoin Core developers";
    }
