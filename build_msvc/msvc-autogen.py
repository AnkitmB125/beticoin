#!/usr/bin/env python3
# Copyright (c) 2016-2019 The beticoin Core developers
...
def main():
    parser = argparse.ArgumentParser(description='beticoin-core msbuild configuration initialiser.')
        ...
        with open(vcxproj_filename + '.in', 'r', encoding='utf-8') as vcxproj_in_file:
            with open(vcxproj_filename, 'w', encoding='utf-8') as vcxproj_file:
                vcxproj_file.write(vcxproj_in_file.read().replace(
                    '@SOURCE_FILES@\n', content))
    copyfile(os.path.join(SOURCE_DIR,'../build_msvc/beticoin_config.h'), os.path.join(SOURCE_DIR, 'config/beticoin-config.h'))
