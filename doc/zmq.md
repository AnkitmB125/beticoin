...
The beticoin Core daemon can be configured to act as a trusted "border
router", implementing the beticoin wire protocol and relay, making
...
The ZeroMQ feature in beticoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building beticoind:
...
For instance:
    ...
    $ beticoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/beticoind.tx.raw \
...
These options can also be provided in beticoin.conf.
...
From the perspective of beticoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into beticoind directly. Furthermore, no information is
...
using. beticoind appends an up-counting sequence number to each
