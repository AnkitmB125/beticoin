# Fuzzing beticoin Core using libFuzzer

## Quickstart guide

To quickly get started fuzzing beticoin Core using [libFuzzer](https://llvm.org/docs/LibFuzzer.html):

```sh
$ git clone https://github.com/beticoin/beticoin
$ cd beticoin/
$ ./autogen.sh
$ CC=clang CXX=clang++ ./configure --enable-fuzz --with-sanitizers=address,fuzzer,undefined
# macOS users: If you have problem with this step then make sure to read "macOS hints for
# libFuzzer" on https://github.com/beticoin/beticoin/blob/master/doc/fuzzing.md#macos-hints-for-libfuzzer
$ make
...
[`process_message`](https://github.com/beticoin/beticoin/blob/master/src/test/fuzz/process_message.cpp) is a fuzzing harness for the [`ProcessMessage(...)` function (`net_processing`)](https://github.com/beticoin/beticoin/blob/master/src/net_processing.cpp). The available fuzzing harnesses are found in [`src/test/fuzz/`](https://github.com/beticoin/beticoin/tree/master/src/test/fuzz).
...
The project's collection of seed corpora is found in the [`beticoin-core/qa-assets`](https://github.com/beticoin-core/qa-assets) repo.

To fuzz `process_message` using the [`beticoin-core/qa-assets`](https://github.com/beticoin-core/qa-assets) seed corpus:

```sh
$ git clone https://github.com/beticoin-core/qa-assets
...
If you find coverage increasing inputs when fuzzing you are highly encouraged to submit them for inclusion in the [`beticoin-core/qa-assets`](https://github.com/beticoin-core/qa-assets) repo.

Every single pull request submitted against the beticoin Core repo is automatically tested against all inputs in the [`beticoin-core/qa-assets`](https://github.com/beticoin-core/qa-assets) repo. Contributing new coverage increasing inputs is an easy way to help make beticoin Core more robust.
...
with certain assembly code from beticoin Core's code. See [developer notes on sanitizers](https://github.com/beticoin/beticoin/blob/master/doc/developer-notes.md#sanitizers)
...
# Fuzzing beticoin Core using afl++

## Quickstart guide

To quickly get started fuzzing beticoin Core using [afl++](https://github.com/AFLplusplus/AFLplusplus):
...
# Fuzzing beticoin Core using Honggfuzz

## Quickstart guide

To quickly get started fuzzing beticoin Core using [Honggfuzz](https://github.com/google/honggfuzz):
...
## Fuzzing the beticoin Core P2P layer using Honggfuzz NetDriver

Honggfuzz NetDriver allows for very easy fuzzing of TCP servers such as beticoin
Core without having to write any custom fuzzing harness. The `beticoind` server
...
```sh
$ mkdir beticoin-honggfuzz-p2p/
$ cd beticoin-honggfuzz-p2p/
$ git clone https://github.com/beticoin/beticoin
$ cd beticoin/
$ ./autogen.sh
$ git clone https://github.com/google/honggfuzz
...
$ git apply << "EOF"
diff --git a/src/beticoind.cpp b/src/beticoind.cpp
...
beticoin Core participates in Google's [OSS-Fuzz](https://github.com/google/oss-fuzz/tree/master/projects/beticoin-core)
program, which includes a dashboard of [publicly disclosed vulnerabilities](https://bugs.chromium.org/p/oss-fuzz/issues/list?q=beticoin-core).
...
because beticoin is a live P2P network, and not just standalone local software,
...
OSS-Fuzz also produces [a fuzzing coverage report](https://oss-fuzz.com/coverage-report/job/libfuzzer_asan_beticoin-core/latest).
